2️⃣ النظام الفرعي: سياسات الدرجات (Subsystem 01)
الهدف

هو دستور الدرجات: من يدرّس؟ وزن كل مكون؟ حد النجاح؟ وأي مكونات إضافية للمدرسة.

الجداول الأساسية
الجدول	وظيفة
lookup_grading_statuses	حالات اعتماد الدرجات (مسودة، مراجعة، معتمد)
grading_policies	أوزان وحدود النجاح لكل صف/مادة/نوع تقييم
grading_policy_custom_components	مكونات إضافية مرنة لكل سياسة
teacher_assignments	تكليف المعلم بالمادة والفصل
أهم التفاصيل التقنية:
1) lookup_grading_statuses

is_final → يمنع التعديل بعد الاعتماد.

is_active → لإظهار/إخفاء الحالة.

2) grading_policies

يحتوي على كل أوزان الاختبارات، الواجب، النشاط، المواظبة، المساهمة.

passing_score → نسبة النجاح.

is_default → سياسة افتراضية للنسخ السنوي.

3) grading_policy_custom_components

يتيح إضافة مكونات مثل: مشروع، سلوك، مهارات، نشاط إثرائي.

يمكن أن تكون يدوية أو آلية.

include_in_monthly وinclude_in_semester → تحكم أين يُحسب المكون.

4) teacher_assignments

يربط المعلم بالصف/المادة/الفصل.

is_primary → معلم أساسي أو بديل.

is_active → يسمح بإيقاف التكليف دون حذف.

أمثلة سريعة:

سياسة درجات: أوزان مختلفة حسب الصف والمادة، مرنة وليست ثابتة.

تكليفات المعلمين: يضمن أن المعلم المخول فقط يمكنه إدخال الدرجات.

مكونات إضافية: يمكن أن تكون نشاط إثرائي، مشروع، سلوك… إلخ.

سير العملية:

اعتماد حالات الدرجات.

إنشاء سياسة لكل صف/مادة/نوع تقييم.

إضافة مكونات مخصصة عند الحاجة.

تكليف المعلمين.

بقية الأنظمة تعتمد هذه السياسة تلقائيًا.

3️⃣ ملاحظات تقييمية

النظام الحالي قوي جدًا ويغطي أغلب المتطلبات الأكاديمية والدرجات.

نقاط التنفيذ الحالية أكثر احترافية من النص المطلوب في كثير من الأمور:

حوكمة وتدقيق.

أوزان مرنة لكل صف ومادة.

فصل الغياب عن الدرجات لتسهيل التحليل.

الفجوات الباقية:

المساهمة المجتمعية لم تُحسب بعد.

ترتيب المدرسة الكامل (rank_in_grade) غير محسوب تلقائي.

كسر التعادل بالاسم غير موجود.

تفاصيل تصدير وتقارير الطباعة (الألوان، الصيغ) خارج نطاق SQL.

حالات الواجب التفصيلية (نفذ/توبع/صحح) غير ممثلة كحالة منفصلة.
